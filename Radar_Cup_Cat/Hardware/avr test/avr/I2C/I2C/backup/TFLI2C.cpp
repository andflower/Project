...
static void onReceive(void (*function)(int))
{
	user_onReceive = function;
}

// sets function called on slave read
static void onRequest(void (*function)(void))
{
...
bool TFLI2C::readReg( uint8_t nmbr, uint8_t addr)
{
  ...
  TFLI2C::requestFrom( ( int)addr, 1, true);
    if( peek() == -1)            // If read error...
    {
      tfStatus = TFL_I2CREAD;         // then set satus code...
      return false;
    }
  regReply = ( uint8_t)read(); // Read the recieved data...
  return true;
}
...
uint8_t TFLI2C::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop) {
	return requestFrom((uint8_t)address, (uint8_t)quantity, (uint32_t)0, (uint8_t)0, (uint8_t)sendStop);
}

uint8_t TFLI2C::requestFrom(uint8_t address, uint8_t quantity)
{
	return requestFrom((uint8_t)address, (uint8_t)quantity, (uint8_t)true);
}

uint8_t TFLI2C::requestFrom(int address, int quantity)
{
	return requestFrom((uint8_t)address, (uint8_t)quantity, (uint8_t)true);
}

uint8_t TFLI2C::requestFrom(int address, int quantity, int sendStop)
{
...
void TFLI2C::printStatus()
{
    ...
    if( tfStatus == TFL_READY)          UART_printString( "READY");
    else if( tfStatus == TFL_SERIAL)    UART_printString( "SERIAL");
    else if( tfStatus == TFL_HEADER)    UART_printString( "HEADER");
    else if( tfStatus == TFL_CHECKSUM)  UART_printString( "CHECKSUM");
    else if( tfStatus == TFL_TIMEOUT)   UART_printString( "TIMEOUT");
    else if( tfStatus == TFL_PASS)      UART_printString( "PASS");
    else if( tfStatus == TFL_FAIL)      UART_printString( "FAIL");
    else if( tfStatus == TFL_I2CREAD)   UART_printString( "I2C-READ");
    else if( tfStatus == TFL_I2CWRITE)  UART_printString( "I2C-WRITE");
    else if( tfStatus == TFL_I2CLENGTH) UART_printString( "I2C-LENGTH");
    else if( tfStatus == TFL_WEAK)      UART_printString( "Signal weak");
    else if( tfStatus == TFL_STRONG)    UART_printString( "Signal strong");
    else if( tfStatus == TFL_FLOOD)     UART_printString( "Ambient light");
    else if( tfStatus == TFL_INVALID)   UART_printString( "No Command");
    else UART_printString( "OTHER");
}
